...
//    let IFLY_AUDIO_SOURCE_STREAM = "-1"
    ...
    override init() {
        ...
        if iFlySpeechRecognizer != nil {
            ...
            iFlySpeechRecognizer?.setParameter("1000", forKey: IFlySpeechConstant.vad_EOS())//后断点检测: 后端点静音检测时间,即用户停止说话多长时间内即认为不再输入,自动停止录音
            iFlySpeechRecognizer?.setParameter("3000", forKey: IFlySpeechConstant.vad_BOS())
            iFlySpeechRecognizer?.setParameter("10000", forKey: IFlySpeechConstant.net_TIMEOUT())//网络等待时间
